...

{
	...
	function index($period = '')
	{
		...
		// DEFINES PAGE TITLE
		$data['title'] = 'Purchase return receipt';
		// DEFINES NAME OF TABLE HEADING
		$data['table_name'] = 'Purchase return receipt  from ' . $date1 . ' to ' . $date2;
		// DEFINES BUTTON NAME ON THE TOP OF THE TABLE
		$data['page_add_button_name'] = 'Purchase return receipt';
		// DEFINES THE TITLE NAME OF THE POPUP
		$data['page_title_model'] = 'Create purchase return receipt';
		...
		$data['purchases_record'] = $this->Crud_model->fetch_record_return_purchase($date1, $date2);
		// DEFINES GO TO MAIN FOLDER FOND INDEX.PHP  AND PASS THE ARRAY OF DATA TO THIS PAGE
		$this->load->view('main/index.php', $data);
	}
	// Purchase_return/add_purchase_return
	function add_purchase_return()
	{
		...
		// DEFINES WHICH PAGE TO RENDER
		$data['main_view'] = 'add_returnpurchase';
		// DEFINES GO TO MAIN FOLDER FOND INDEX.PHP  AND PASS THE ARRAY OF DATA TO THIS PAGE
		$this->load->view('main/index.php', $data);
	}
	// Purchase_return/edit_purchase_return
	function edit_purchase_return($purchase_id)
	{
		$this->load->model('Crud_model');
		// DEFINES PAGE TITLE
		$data['title'] = 'Edit Purchase return';
		// PARENT DATA OF REFUND
		$data['parent_row'] = $this->Crud_model->fetch_record_by_id('mp_purchase_return', $purchase_id);
		// CHILD DATA OF REFUND
		$data['child_row'] = $this->Crud_model->fetch_attr_record_by_id('mp_sub_purchase_return', 'sales_id', $purchase_id);
		// DEFINES TO LOAD THE CATEGORY LIST FROM DATABSE TABLE mp_supplier
		$data['product_list'] = $this->Crud_model->fetch_record('mp_product', NULL);
		// DEFINE TO FETCH THE LIST OF SUPPLIER
		$data['payee_list'] = $this->Crud_model->fetch_payee_record('all', 'status');
		...
		// DEFINES WHICH PAGE TO RENDER
		$data['main_view'] = 'edit_purchase_return';
		// DEFINES GO TO MAIN FOLDER FOND INDEX.PHP  AND PASS THE ARRAY OF DATA TO THIS PAGE
		$this->load->view('main/index.php', $data);
	}
	// Purchase_return/add_returnpurchase
	// USED TO ADD PURCHASE RETURN INTO TABLE
	function add_returnpurchase()
	{
		$this->load->model('Crud_model');
		...
		$total_received = html_escape($this->input->post('received'));
		$invoicemessage = html_escape($this->input->post('invoicemessage'));
		$memo = html_escape($this->input->post('memo'));
		$attachment = $this->Crud_model->do_upload_picture("attachment", "./uploads/purchase_return/");
		...
		else
		{
			if (count($product) > 0 AND $total_bill > 0)
			{
				...
				$result = $this->PR_model->add_purchase_return_transaction($args);
				if ($result != NULL)
				{
					$array_msg = array(
	...
	function update_purchase_return()
	{
		...
		$single_tax = html_escape($this->input->post('single_tax'));
		$total_bill = html_escape($this->input->post('total_bill'));
		$total_received = html_escape($this->input->post('total_received'));
		$invoicemessage = html_escape($this->input->post('invoicemessage'));
		$memo = html_escape($this->input->post('memo'));
		...
		{
			$array_msg = array(
				'msg' => '<i style="color:#fff" class="fa fa-check-circle-o" aria-hidden="true"></i> Please select bank account',
				'alert' => 'danger'
			);
			$this->session->set_flashdata('status', $array_msg);
			redirect(base_url('purchase_return'));
		...
		else
		{
			if (count($product) > 0)
			{
				...
				{
					// DEFINES TO DELETE IMAGE FROM FOLDER PARAMETER REQUIRES ARRAY OF IMAGE PATH AND ID
					$this->Crud_model->delete_image_custom('./uploads/purchase_return/', $receipt_id, 'attachment', 'mp_purchase_return');
				}
				// DEFINES CALL THE FUNCTION OF insert_data FORM Crud_model CLASS
				$result = $this->PR_model->update_purchase_return_transaction($args);
				if ($result != NULL)
				{
					$array_msg = array(
	...
	function popup($page_name = '', $param = '')
	{
		...
		{
			$data['redirect_link'] = 'purchase_return/add_purchase_return';
	...
	// Purchase_return/view_attachment
	// USED TO VIEW ATTACHMENT
	function view_attachment($id)
	{
		$this->load->model('Crud_model');
		$data['title'] = 'Purchase return | Attachment';
		// FETCH THE PROVIED ID OF ATTACHMENT TO VIEW OR PRINT
		$purchase_data = $this->Crud_model->fetch_record_by_id('mp_purchase_return', $id);
		// CONTROLLER LINK TO REDIRECT
		$data['controller_link'] = 'purchase_return';
		// BREADCRUMB NAME OR TITLE
		$data['controller_name'] = 'Purchase return';
		// ATACHMENT IMAGE PATH
		$data['img_path'] = 'uploads/purchase_return/' . $purchase_data[0]->attachment;
		// DEFINES WHICH PAGE TO RENDER
